...
- Local model distributions under `Models/` (each typically includes `meta.yaml`, `*.mlmodelc`, tokenizer assets, etc.).
...
Place one or more model bundles under `Models/`.

Conventions:
- Model bundles are local-only artifacts and are ignored by git.
- The server will default to discovering models under `./Models` (repo-relative), with a flag/env override when implemented.
...
# Clone a model repo into `Models/` (example naming)
git clone https://huggingface.co/anemll/<model-repo> Models/<model-repo>
cd Models/<model-repo>
git lfs pull
